# -*- generated by 1.0.12 -*-
import da
PatternExpr_510 = da.pat.TuplePattern([da.pat.ConstantPattern('r'), da.pat.FreePattern('round_i')])
PatternExpr_524 = da.pat.TuplePattern([da.pat.ConstantPattern('M'), da.pat.FreePattern('p'), da.pat.FreePattern('z'), da.pat.FreePattern('Zv')])
_config_object = {}
import sys
import math
import functools
...
class P(da.DistProcess):
    ...
    def __init__(self, procimpl, props):
        ...
        self._events.extend([da.pat.EventPattern(da.pat.ReceivedEvent, '_PReceivedEvent_0', PatternExpr_510, sources=None, destinations=None, timestamps=None, record_history=None, handlers=[self._P_handler_509]), da.pat.EventPattern(da.pat.ReceivedEvent, '_PReceivedEvent_1', PatternExpr_524, sources=None, destinations=None, timestamps=None, record_history=None, handlers=[self._P_handler_523])])

    def setup(self, neighbors, sources, clique, idmapping, G, delta, h, r_max, **rest_1240):
        super().setup(neighbors=neighbors, sources=sources, clique=clique, idmapping=idmapping, G=G, delta=delta, h=h, r_max=r_max, **rest_1240)
        ...
        self._state.r = 0
        self._state.r_max = self._state.r_max
    ...
    def run(self):
        self.initiate()
        while (self._state.r <= self._state.r_max):
            self.msgsending()
            self.sync()
    ...
    def sync(self):
        ...
        super()._label('_st_label_494', block=False)
        _st_label_494 = 0
        while (_st_label_494 == 0):
            _st_label_494 += 1
            if (self._state.rcount[self._state.r] == self._state.pcount):
                self._state.r += 1
                _st_label_494 += 1
            else:
                super()._label('_st_label_494', block=True)
                _st_label_494 -= 1
    ...
    def _P_handler_509(self, round_i):
        self._state.rcount[round_i] += 1
    _P_handler_509._labels = None
    _P_handler_509._notlabels = None

    def _P_handler_523(self, p, z, Zv):
        self.output('lists', self._state.z_list)
        kappa = ((z[0] + (self._state.G.getWeight(self._state.idmapping[p], self._state.idmapping[self._id]) * self.gamma(len(self._state.sources), self._state.h, self._state.delta))) + 1)
        ...
        for entry in self._state.z_list:
            if (entry[4] and (entry[3] == z[3])):
                z_star = entry
        if (z[4] and (l <= self._state.h) and ((d < self._state.d_star_dict[z[3]]) or ((d == z_star[1]) and (kappa < z_star[0])) or ((d == z_star[1]) and (kappa == z_star[0]) and (new_z[5] < z_star[5])))):
            ...
            if (num_of_entries_z_list < Zv):
                ...
                for i in range((index_z + 1), len(self._state.z_list)):
                    ...
                    if ((z_temp[3] == new_z[3]) and (not z_temp[4])):
                        self._state.z_list.remove(z_temp)
                        break
    _P_handler_523._labels = None
    _P_handler_523._notlabels = None
...
class Node_(da.NodeProcess):
    ...
    def run(self):
        ...
        for (i, p) in enumerate(ps):
            ...
            print(neighbors)
            self._setup({p}, (neighbors, sources, [ps[i0] for (i0, p0) in enumerate(ps)], idmapping, graph, delta, hop, r_max))
        self._start(ps)
