# -*- generated by 1.0.12 -*-
import da
PatternExpr_513 = da.pat.TuplePattern([da.pat.ConstantPattern('r'), da.pat.FreePattern('round_i')])
PatternExpr_527 = da.pat.TuplePattern([da.pat.ConstantPattern('M'), da.pat.FreePattern('p'), da.pat.FreePattern('z'), da.pat.FreePattern('Zv')])
_config_object = {}
import sys
import math
import functools
...
class G():
    ...
    def neighbor(self, p):
        ...
        for i in range(self.np):
            if ((not (neighbor_weights[i] == 0)) or (not (neighbor_weights[i] == (- 2)))):
...
class P(da.DistProcess):
    ...
    def __init__(self, procimpl, props):
        ...
        self._events.extend([da.pat.EventPattern(da.pat.ReceivedEvent, '_PReceivedEvent_0', PatternExpr_513, sources=None, destinations=None, timestamps=None, record_history=None, handlers=[self._P_handler_512]), da.pat.EventPattern(da.pat.ReceivedEvent, '_PReceivedEvent_1', PatternExpr_527, sources=None, destinations=None, timestamps=None, record_history=None, handlers=[self._P_handler_526])])

    def setup(self, neighbors, sources, clique, idmapping, G, delta, h, **rest_1187):
        super().setup(neighbors=neighbors, sources=sources, clique=clique, idmapping=idmapping, G=G, delta=delta, h=h, **rest_1187)
    ...
    def _P_handler_512(self, round_i):
        self._state.rcount[round_i] += 1
    _P_handler_512._labels = None
    _P_handler_512._notlabels = None

    def _P_handler_526(self, p, z, Zv):
            ...
            if (num_of_entries_z_list < Zv):
                ...
                for i in range((index_z + 1), len(self._state.z_list)):
                    ...
                    if ((z_temp[3] == new_z[3]) and (not z_temp[4])):
                        self._state.z_list.remove(z_temp)
                        break
    _P_handler_526._labels = None
    _P_handler_526._notlabels = None
