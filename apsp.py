# -*- generated by 1.0.12 -*-
import da
PatternExpr_509 = da.pat.TuplePattern([da.pat.ConstantPattern('r'), da.pat.FreePattern('round_i')])
PatternExpr_523 = da.pat.TuplePattern([da.pat.ConstantPattern('M'), da.pat.FreePattern('p'), da.pat.FreePattern('z'), da.pat.FreePattern('Zv')])
_config_object = {}
import sys
import math
import functools
...
class P(da.DistProcess):
    ...
    def __init__(self, procimpl, props):
        ...
        self._events.extend([da.pat.EventPattern(da.pat.ReceivedEvent, '_PReceivedEvent_0', PatternExpr_509, sources=None, destinations=None, timestamps=None, record_history=None, handlers=[self._P_handler_508]), da.pat.EventPattern(da.pat.ReceivedEvent, '_PReceivedEvent_1', PatternExpr_523, sources=None, destinations=None, timestamps=None, record_history=None, handlers=[self._P_handler_522])])

    def setup(self, neighbors, sources, clique, idmapping, G, delta, h, **rest_1186):
        super().setup(neighbors=neighbors, sources=sources, clique=clique, idmapping=idmapping, G=G, delta=delta, h=h, **rest_1186)
        ...
        self._state.pcount = (len(self._state.clique) + 1)
    ...
    def sync(self):
        ...
        while (_st_label_491 == 0):
            _st_label_491 += 1
            if (self._state.rcount[self._state.r] == (self._state.pcount - 1)):
                self._state.r += 1
                _st_label_491 += 1
            else:
                super()._label('_st_label_491', block=True)
                _st_label_491 -= 1
    ...
    def _P_handler_508(self, round_i):
        self._state.rcount[round_i] += 1
    _P_handler_508._labels = None
    _P_handler_508._notlabels = None

    def _P_handler_522(self, p, z, Zv):
            ...
            if (num_of_entries_z_list < Zv):
                ...
                for i in range((index_z + 1), len(self._state.z_list)):
                    ...
                    if ((z_temp[3] == new_z[3]) and (not z_temp[4])):
                        self._state.z_list.remove(z_temp)
                        break
    _P_handler_522._labels = None
    _P_handler_522._notlabels = None
...
class Node_(da.NodeProcess):
    ...
    def run(self):
        ...
        for (i, p) in enumerate(ps):
            ...
            print(neighbors)
            self._setup({p}, (neighbors, sources, [ps[i0] for (i0, p0) in enumerate(ps) if (not (i0 == i))], idmapping, graph, delta, hop))
        self._start(ps)
